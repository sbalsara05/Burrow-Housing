...
const PropertyMap: React.FC<PropertyMapProps> = ({
  ...
  // Add test markers to verify map functionality
  const addTestMarkers = () => {
    if (!mapInstanceRef.current) return;

    console.log('Adding test markers...');

    // Test marker locations around Boston
    const testLocations = [
      { lat: 42.3601, lng: -71.0589, title: 'Boston Center' },
      { lat: 42.3505, lng: -71.0763, title: 'Back Bay' },
      { lat: 42.3355, lng: -71.0695, title: 'South End' }
    ];
    ...
    testLocations.forEach((location, index) => {
      ...
      marker.addListener('click', () => {
        console.log('Test marker clicked:', location.title);
      });
    });
    ...
    properties.forEach((property, index) => {
      ...
      try {
        const marker = new google.maps.Marker({
          ...
          label: `$${Math.round((property.overview?.rent || 0) / 1000)}k`
        });
  ...
  return (
    <div style={{ position: 'relative', height: mapHeight }} className={className}>
      ...
      {/* Debug overlay */}
      <div className="position-absolute top-0 start-0 m-2 p-2 bg-dark bg-opacity-75 text-white rounded small" style={{ maxWidth: '300px', fontSize: '11px' }}>
        <div><strong>Debug Info:</strong></div>
        <div>Map Ready: {isMapReady ? '✅' : '❌'}</div>
        <div>Google Maps: {window.google?.maps ? '✅' : '❌'}</div>
        <div>Properties: {debugInfo}</div>
        <div>API Key: {API_KEY ? '✅' : '❌'}</div>
        {properties.length > 0 && (
          <div>
            <strong>Sample Property:</strong>
            <pre style={{ fontSize: '10px', maxHeight: '100px', overflow: 'auto' }}>
              {JSON.stringify({
                id: properties[0]._id,
                location: properties[0].addressAndLocation?.location,
                title: properties[0].overview?.title,
                rent: properties[0].overview?.rent
              }, null, 1)}
            </pre>
          </div>
        )}
